...
# Read .ply file
INPUT_FILE = "/datasets/project/Coarsed_version.ply"
...
def triangle_geometries(points, tri_simplices):
    ...
    triangles = np.array([point[:][:2] for point in triangles])
...
def triangles_to_holes(points, tri_simplices, big_triangles,
    ...
    print(points_indexed[tri_simplices[big_triangles[0]]].reshape(-1,3))
    if len(big_triangles) == 1:
        holes = [Polygon(points_indexed[tri_simplices[big_triangles[0]]].reshape(-1,3))]
...
def fill_holes(points, max_circumradius=0.6, max_ratio_radius_area=0.2,
...
def make_plane(points,height):
    pts=points
    for i in range(int(pts.size/3)):
        point=pts[i]
        point[2]=height
        pts[i]=point
    return pts
...
if __name__ == "__main__":
    ...
    new_points = pcd.select_by_index(inliers)
    print("Before filling:", new_points)
    o3d.io.write_point_cloud(SAVE_PATH+"plane.ply", new_points)
    filled_holes = fill_holes(pcd.points)
    npt=np.asarray(new_points.points)
    npt=make_plane(npt,filled_holes[0][2])
    filled_holes= np.concatenate((npt,filled_holes))
    new_points.points = o3d.utility.Vector3dVector(filled_holes)
    print("After filling:",new_points)
    o3d.io.write_point_cloud(SAVE_PATH + "filled.ply", new_points)
